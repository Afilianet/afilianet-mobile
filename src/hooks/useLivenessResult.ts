...
/**
 * Mirrors useFaceMatchResult.ts exactly (Phase 9E.2's polling pattern,
 * applied to AWS liveness): the latest liveness session for a step, polled
 * while a session is in flight. A 404 (no session created yet) is treated
 * as "no result", not an error.
 *
 * Polling stops automatically once `status` reaches a terminal value
 * (completed/failed) or there's no session yet to poll for, and stops
 ...
 *
 * A `completed` session (any verdict) always calls
 ...
 * useFaceMatchResult uses. This only fires once per newly-seen completed
 * session (tracked by id), not on every poll tick. A technical `failed`
 * session never touches compliance state at all (never calls
 * attemptStepAsProvider, never consumes attempt_count), so no invalidation
